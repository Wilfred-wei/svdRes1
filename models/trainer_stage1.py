...
class Trainer_stage1:
    def __init__(self, opt):
        ...
        # SVD 正交损失系数 (Effort 论文思想)
        self.orth_lambda = getattr(opt, 'orth_lambda', 0.1)  # 默认 0.1
    ...
    def train_epoch(self, dataloader: DataLoader, criterion):
        ...
        for batch_idx, (data, target) in enumerate(tqdm(dataloader)):
            ...
            with autocast():
                ...
                # SVD 正交损失 (Effort 论文思想)
                # 迫使 WSGM 特征与 CLIP 语义子空间正交
    ...
    def validate_epoch(self, dataloader: DataLoader, criterion, epoch: int, writer: SummaryWriter = None):
        ...
        for data, target in tqdm(dataloader):
            ...
            with torch.no_grad():
                with autocast():
                    # 启用正交损失计算
                    pre, _, orth_loss = self.model(data, return_orth_loss=True)

                    # 分类损失
                    loss_cls = criterion(pre.squeeze(1), target.type(torch.float32))
        ...
        if writer is not None:
            ...
            writer.add_scalar('Loss_Orth/Validation', running_orth_loss / num_batches, epoch)
            writer.add_scalar('Accuracy', acc, epoch)
            writer.add_scalar('Average Precision', ap, epoch)

        return running_loss / num_batches, acc, ap
